                            ...
                            <tr class="{{ $key % 2 === 0 ? 'bg-gray-100' : 'bg-white' }} hover:bg-gray-200">
                                ...
                                <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <button class="openModalButton" data-object-id="{{ $patient_activity->id }}" data-object-type="patient_activity">
                                        ...
                                        <div class="confirmationModal hidden fixed inset-0 z-50 items-center justify-center bg-black bg-opacity-50" data-object-id="{{ $patient_activity->id }}" data-object-type="patient_activity">
                                            <div class="bg-white p-6 rounded shadow-lg w-1/3">
                                                <p>¿Estás seguro de que deseas eliminar esta Actividad del Paciente?</p>
                                                <div class="flex justify-end space-x-4 mt-4">
                                                    <button class="cancelButton bg-gray-500 text-white py-2 px-4 rounded hover:bg-gray-600" data-object-id="{{ $patient_activity->id }}">Cancelar</button>
                                                    <!-- Formulario de eliminación dentro del modal -->
                                                    <form action="{{ route('patient-activities.destroy', $patient_activity->id) }}" method="POST" class="deleteForm">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="confirmButton bg-red-500 text-white py-2 px-4 rounded hover:bg-red-600" data-object-id="{{ $patient_activity->id }}">Confirmar</button>
                                                    </form>
                                                </div>
                                            </div>
                                        </div>
