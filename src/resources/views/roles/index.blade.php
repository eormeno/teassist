<x-event-layout>
                                        ...
                                        <tr class="{{ $key % 2 === 0 ? 'bg-gray-100' : 'bg-white' }} hover:bg-gray-200">
                                            ...
                                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500">
                                                <div class="flex items-center space-x-2">
                                                    @if ($role->name !== 'root')
                                                        <a href="{{ route('roles.show', $role->id) }}" class="text-indigo-600 hover:text-indigo-900">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                                                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                                                            </svg>
                                                        </a>
                                                        <a href="{{ route('roles.edit', $role->id) }}" class="text-[#A0A8FF] hover:text-[#6572fc]">
                                    <!-- Icono de edición -->
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                                        ...
                                                        <button
                                                            ...
                                                            x-data
                                                            @click="$dispatch('open-modal', { roleId: {{ $role->id }} })"
                                                        >
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
    ...
    <!-- Modal -->
    <div x-data="{ showModal: false, roleId: null }" @open-modal.window="showModal = true; roleId = $event.detail.roleId">
        <template x-if="showModal">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center">
                <div class="bg-white rounded-lg p-6 shadow-lg max-w-sm">
                    ...
                    <p class="mb-4">¿Estás seguro de que quieres eliminar este rol?</p>
                    <div class="flex justify-end space-x-2">
                        ...
                        <form :action="'/roles/' + roleId" method="POST">
                            @csrf
                            @method('DELETE')
