<x-crud-layout>
    ...
    <x-button onclick="location.href='{{ route('activities.create') }}'" class="inline-flex items-center px-4 py-2 mb-2 font-semibold text-xs disabled:opacity-25 transition ease-in-out duration-150">
        Nueva actividad
    </x-button>
                        ...
                        <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500">
                            <div class="flex items-center space-x-2">
                                ...
                                <button
                                    ...
                                    x-data
                                    @click="$dispatch('open-modal', { activityId: {{ $activity->id }} })">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="size-6">
                                        <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <!-- Modal -->
    <div x-data="{ showModal: false, activityId: null }" @open-modal.window="showModal = true; activityId = $event.detail.activityId">
        <template x-if="showModal">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center">
                <div class="bg-white rounded-lg p-6 shadow-lg max-w-sm">
                    ...
                    <p class="mb-4">¿Estás seguro de que quieres eliminar esta actividad?</p>
                    <div class="flex justify-end space-x-2">
                        ...
                        <form :action="'/activities/' + activityId" method="POST">
                            @csrf
                            @method('DELETE')
